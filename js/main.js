...
function renderBoard(board) {
    ...
    document.title = 'Tacos Taco | ' + currentBoard().name;
...
class Board {
    ...
    if (this.cards.length === 0) {
        ...
        // For additional cards, you can customize the titles based on your requirements
        _cardTitle = `Custom Title ${this.cards.length + 1}`;
    }
}
...
function loadData() {
    ...
    } else if(appData.currentBoard == 0){
        let _defaultBoard = new Board("Resources", 'b0', {'theme': null});
        appData.boards.push(_defaultBoard);
    }
    else if(appData.currentBoard==1){
        let _defaultBoard= new Board("To Do",'b1',{'theme':null});
        appData.boards.push(_defaultBoard);
    }
    else if(appData.currentBoard==2){
        let _defaultBoard= new Board("Doing",'b2',{'theme':null});
        appData.boards.push(_defaultBoard);
    }
    else if(appData.currentBoard==3){
        let _defaultBoard= new Board("Done",'b3',{'theme':null});
        appData.boards.push(_defaultBoard);
    }
    listBoards();
}
...
e_deleteButton.addEventListener('click', () => {
    createConfirmDialog('Are you sure to delete this board?', () => {
        ...
        if (appData.boards.length === 0) {
            let _defaultBoard = new Board("Taco's Taco", 'b0', {'theme': null});
            appData.boards.push(_defaultBoard);
